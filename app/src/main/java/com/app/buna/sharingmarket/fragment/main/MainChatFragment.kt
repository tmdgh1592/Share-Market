...
class MainChatFragment : Fragment() {
    ...
    fun initView() {
        ...
        vm.getChatRoomList(object : IFirebaseGetChatRoomCallback {
            override fun complete(chatModels: ArrayList<ChatModel>) {
                ...
                vm.chatModels.clear()
                vm.chatModels.addAll(chatModels.sortedByDescending { it.lastTimestamp }) // chatRoomList -> 채팅방에 사용할 데이터 리스트 && 가장 최근 채팅을 가져오기 위해 lastTimeStamp 순으로 오름차순 정렬
                ...
                vm.chatModels.forEach { chatModel ->
                    ...
                    if (destUid != null) {
                        vm.getUserModel(destUid) { userModel -> // 상대방 Uid를 찾아서 이 Uid로 해당 유저 정보 가져오기
                            ...
                            if (++count == chatModels.size){ // 가져올 데이터를 모두 가져왔다면
                                // Recycler View에게 알려줌
